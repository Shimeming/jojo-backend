...
- VENUE：代表需要預約的校內場地。可為校內教室、討論室、運動場地。系統記錄場地名稱、可容納人數、可借時段（例如 8–21 點）、及場地狀態（可用/維修中）。
...
`USER、ADMIN_USER、USER_GROUP、EVENT、JOIN_RECORD、VENUE、VENUE_BOOKING、GROUP、PREFERENCE、TYPE`。
...
- `VENUE`
  - 主鍵（PK）：Venue_id
  - 屬性：Name, Building, Floor, Capacity, Open_time, Close_time, Status
...
- 主鍵 : 所有的資料表(`USER`, `ADMIN_USER`, `GROUP`, `TYPE`, `USER_GROUP`, `EVENT`, `JOIN_RECORD`, `VENUE`, `VENUE_BOOKING`, `PREFERENCE`) 都已定義了主鍵(PK)。
...
- `PREFERENCE`： 僅儲存 `USER` 和 `TYPE` 之間的M:N關係。

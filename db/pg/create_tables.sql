...
-- TYPE
CREATE TABLE IF NOT EXISTS jojo.TYPE (name VARCHAR(50) PRIMARY KEY);
-- EVENT
CREATE TABLE IF NOT EXISTS jojo.EVENT (
  ...
  FOREIGN KEY (type_name) REFERENCES jojo.TYPE(name) ON DELETE CASCADE,
...
-- VENUE_BOOKING
CREATE TABLE IF NOT EXISTS jojo.VENUE_BOOKING (
  event_id BIGINT PRIMARY KEY,
  venue_id BIGINT NOT NULL,
  book_datetime TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (event_id) REFERENCES jojo.EVENT(event_id) ON DELETE CASCADE,
  FOREIGN KEY (venue_id) REFERENCES jojo.VENUE(venue_id) ON DELETE CASCADE
);
-- PREFERENCE
